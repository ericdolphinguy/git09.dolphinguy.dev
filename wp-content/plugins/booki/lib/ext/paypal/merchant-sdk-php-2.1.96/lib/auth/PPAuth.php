...
class PPAuthSignature {
	...
	public function genSign($key, $secret, $token, $tokenSecret, $httpMethod, $endpoint) {
		...
		$authConsumer = new PPOAuthConsumer($key, $secret, NULL);
		$authToken = NULL;
		$authToken = new PPOAuthToken($token, $tokenSecret);
	...
	public static function generateFullAuthString($key, $secret, $token, $tokenSecret, $httpMethod, $endpoint) {
		$authSignature = new PPAuthSignature();
 ...
 */
class PPOAuthException extends Exception {
	// pass
}

class PPOAuthConsumer {
	public $key;
	public $secret;
	...
	function __toString() {
		return "PPOAuthConsumer[key=$this->key,secret=$this->secret]";
	}
}
...
abstract class PPOAuthSignatureMethod {
	 ...
	 * the encoding is handled in PPOAuthRequest when the final
	 * request is serialized
	 * @param PPOAuthRequest $request
	 * @param PPOAuthConsumer $consumer
	 * @param PPOAuthToken $token
	 * @return string
	 */
 ...
 */
class PPOAuthSignatureMethod_PLAINTEXT extends PPOAuthSignatureMethod {
 ...
 */
abstract class PPOAuthSignatureMethod_RSA_SHA1 extends PPOAuthSignatureMethod {
	public function get_name() {
		return "RSA-SHA1";
	}
...
class PPOAuthRequest {
	...
	function __construct($http_method, $http_url, $parameters=NULL) {
		...
		$parameters = array_merge( PPOAuthUtil::parse_parameters(parse_url($http_url, PHP_URL_QUERY)), $parameters);
	...
	public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) {
		...
		if (!$parameters) {
			...
			$request_headers = PPOAuthUtil::get_headers();

			// Parse the query-string to find GET parameters
			$parameters = PPOAuthUtil::parse_parameters($_SERVER['QUERY_STRING']);
			...
			&& strstr($request_headers['Content-Type'],
				...
				$post_data = PPOAuthUtil::parse_parameters(
			...
			if (isset($request_headers['Authorization']) && substr($request_headers['Authorization'], 0, 6) == 'OAuth ') {
				$header_parameters = PPOAuthUtil::split_header(
	...
	public function get_signable_parameters() {
		...
		return implode('&', $res);
		//return PPOAuthUtil::build_http_query($params);
	}
	...
	public function get_signature_base_string() {
		...
		$parts = PPOAuthUtil::urlencode_rfc3986($parts);
	...
	public function to_postdata() {
		return PPOAuthUtil::build_http_query($this->parameters);
	}
	...
	public function to_header($realm=null) {
		$first = true;
		if($realm) {
			$out = 'Authorization: OAuth realm="' . PPOAuthUtil::urlencode_rfc3986($realm) . '"';
			$first = false;
		} else
		...
		foreach ($this->parameters as $k => $v) {
			...
			if (is_array($v)) {
				throw new PPOAuthException('Arrays not supported in headers');
			}
			$out .= ($first) ? ' ' : ',';
			$out .= PPOAuthUtil::urlencode_rfc3986($k) .
              '="' .
			PPOAuthUtil::urlencode_rfc3986($v) .
              '"';
			$first = false;
		}
		return $out;
	...
	public function sign_request($signature_method,$consumer, $token) {
		...
		if(count($msg))
		{
			throw new PPOAuthException('Enter valid '. implode(',',$msg));
		}
		$this->set_parameter(
...
class PPOAuthServer {
	...
	private function get_version(&$request) {
		...
		if ($version !== $this->version) {
			throw new PPOAuthException("OAuth version '$version' not supported");
		}
		return $version;
	}
	...
	private function get_signature_method($request) {
		...
		if (!$signature_method) {
			...
			throw new PPOAuthException('No signature method parameter. This parameter is required');
		}

		if (!in_array($signature_method,
		array_keys($this->signature_methods))) {
			throw new PPOAuthException(
	...
	private function get_consumer($request) {
		...
		if (!$consumer_key) {
			throw new PPOAuthException("Invalid consumer key");
		}

		$consumer = $this->data_store->lookup_consumer($consumer_key);
		if (!$consumer) {
			throw new PPOAuthException("Invalid consumer");
		}
	...
	private function get_token($request, $consumer, $token_type="access") {
		...
		if (!$token) {
			throw new PPOAuthException("Invalid $token_type token: $token_field");
		}
		return $token;
	}
	...
	private function check_signature($request, $consumer, $token) {
		...
		if (!$valid_sig) {
			throw new PPOAuthException("Invalid signature");
		}
	}
	...
	private function check_nonce($consumer, $token, $nonce, $timestamp) {
        ...
        if ($found) {
        	throw new PPOAuthException("Nonce already used: $nonce");
        }
	}
...
class PPOAuthUtil {
	public static function urlencode_rfc3986($input) {
		if (is_array($input)) {
			return array_map(array('PPOAuthUtil', 'urlencode_rfc3986'), $input);
	...
	public static function split_header($header, $only_allow_oauth_parameters = true) {
		...
		if (preg_match_all('/('.($only_allow_oauth_parameters ? 'oauth_' : '').'[a-z_-]*)=(:?"([^"]*)"|([^,]*))/', $header, $matches)) {
			foreach ($matches[1] as $i => $h) {
				$params[$h] = PPOAuthUtil::urldecode_rfc3986(empty($matches[3][$i]) ? $matches[4][$i] : $matches[3][$i]);
			}
	...
	public static function parse_parameters( $input ) {
		...
		foreach ($pairs as $pair) {
			...
			$parameter = PPOAuthUtil::urldecode_rfc3986($split[0]);
			$value = isset($split[1]) ? PPOAuthUtil::urldecode_rfc3986($split[1]) : '';
	...
	public static function build_http_query($params) {
		...
		$keys = PPOAuthUtil::urlencode_rfc3986(array_keys($params));
		$values = PPOAuthUtil::urlencode_rfc3986(array_values($params));
